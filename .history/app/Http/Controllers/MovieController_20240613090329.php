...
class MovieController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('movie_search')) {
            ...
            $movies_recommend = Movie::where('released', '=', '1')
                ->whereRaw('LOWER(name) NOT LIKE ?', ['%' . strtolower($name_search) . '%'])
                ->get();
            return View('Home', compact('movies_recommend', 'movies_search', 'name_search'));
        }
